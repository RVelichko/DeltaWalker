...
static const size_t DEFAULT_ASYNC_THRED_STEP = 100;


/**
 * \brief Посекундный таймер с корректной остановкой потока.
 */
template <size_t ASYNC_THRED_STEP = DEFAULT_ASYNC_THRED_STEP>
class Timer {
...
public:
    /**
     * \brief Шаблонный конструктор объекта таймера.
            ...
            _thread = std::make_shared<std::thread>([=] {
                ...
                while (_is_run.test_and_set(std::memory_order_acquire)) {
                    ...
                    std::this_thread::sleep_for(
                        chr::milliseconds(std::min(ASYNC_THRED_STEP, mlsleep)));
                    // Проверить готовность таймера
                    chr::nanoseconds diff =
                        chr::duration_cast<chr::nanoseconds>(chr::steady_clock::now() - start);
                    if (mlsleep <= chr::duration_cast<chr::milliseconds>(diff).count()) {
                        task();
