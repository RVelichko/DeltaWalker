...
/**
 * \brief Базовый класс контроллера комнат, определяет общие методы управления комнатами для 2
 *        классов мемберов
 */
...
public:
    ...
    /**
     * \brief Метод поиска комнаты по идентификатору
    ...
    TRoomMembers getRoom(const std::string &room_id) {
        ...
        if (iter not_eq _rooms.end()) {  ///< При наличии искомой комнаты проинициализировать
                                         ///< возвращаемую структуру комнаты
            members = iter->second;
        }
        return members;
    }
    ...
    /**
     * \brief Метод удаления комнаты по идентификатору
     * \param room_id  Идентификатор искомой комнаты
     */
    void deleteRoom(size_t room_id) { _rooms.erase(room_id); }

    /**
     * \brief Шаблонный метод установки данных в структуру комнаты
     * \param room_id  Идентификатор искомой комнаты
    ...
    /**
     * \brief Возврат копии структуры комнат
     */
    MapRooms getRooms() {
...
/**
 * \brief Шаблонная структура - хелпер, для определения неопределённых шаблонных типов структур
 * комнаты
 */
template <class TDataType>
struct RoomData {
    typedef std::tuple<size_t, size_t, TDataType> SingleMembers;  ///< Комната 1 : 1
    typedef std::tuple<size_t, std::set<size_t>, TDataType> SingleMultyMembers;  ///< Комната 1 : N
    typedef std::tuple<std::set<size_t>, std::set<size_t>, TDataType>
        MultyMembers;  ///< Комната N : N
};
...
/**
 * \brief Шаблонный класс, комнаты 1 : 1
 */
template <class TDataType, class TRoomMembers = typename RoomData<TDataType>::SingleMembers>
class RoomController : public BaseRoomController<TRoomMembers> {
...
/**
 * \brief Шаблонный класс, комнаты 1 : N
 */
template <class TDataType>
class RoomController<TDataType, typename RoomData<TDataType>::SingleMultyMembers>
    : public BaseRoomController<typename RoomData<TDataType>::SingleMultyMembers> {
    typedef BaseRoomController<typename RoomData<TDataType>::SingleMultyMembers> Base;

public:
    RoomController() { std::get<0>(Base::_null_members) = 0; }

    virtual ~RoomController() {}

    typename RoomData<TDataType>::SingleMultyMembers addToRoom(const std::string &room_id,
                                                               const Members &mbr) {
        typename RoomData<TDataType>::SingleMultyMembers members;
        auto iter = Base::_rooms.find(room_id);
        if (iter not_eq Base::_rooms.end()) {
            size_t id_0 = std::get<0>(mbr);
            if (id_0 not_eq 0) {
    ...
    typename RoomData<TDataType>::SingleMultyMembers deleteFromRoom(const std::string &room_id,
                                                                    const Members &mbr) {
        typename RoomData<TDataType>::SingleMultyMembers members;
        auto iter = Base::_rooms.find(room_id);
        if (iter not_eq Base::_rooms.end()) {
...
/**
 * \brief Шаблонный класс, комнаты N : N
 */
template <class TDataType>
...
public:
    ...
    typename RoomData<TDataType>::MultyMembers addToRoom(const std::string &room_id,
                                                         const Members &mbr) {
    ...
    typename RoomData<TDataType>::MultyMembers deleteFromRoom(const std::string &room_id,
                                                              const Members &mbr) {
        typename RoomData<TDataType>::MultyMembers members;
        auto iter = Base::_rooms.find(room_id);
        if (iter not_eq Base::_rooms.end()) {
