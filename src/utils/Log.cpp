...
static std::string TimevalToStr(PTimevalWrap tvw, bool simple_tp = false,
                                const std::string& format_ = "%d.%m.%Y-%H:%M:%S",
                                bool out_usecs = true) {
    if (simple_tp) {
        if (out_usecs) {
            return std::to_string(tvw->tv.tv_sec) + "." + std::to_string(tvw->tv.tv_usec);
        }
        return std::to_string(tvw->tv.tv_sec);
    }
    ...
    if (out_usecs) {
        return std::to_string(t) + "|" + std::string(buf) + "." + std::to_string(tvw->tv.tv_usec);
    }
...
static std::string LogFileName(size_t i) {
    ...
    gettimeofday(&tvw->tv, nullptr);
    return (LOG_FILE_PATH + TimevalToStr(tvw, false, "-[%d.%m.%Y-%H.%M]-", false) +
            std::to_string(i) + ".log");
}
...
void Log::execute() {
    while (true) {
        ...
        std::stringstream ss;
        ss << (++_file_line_number) << ".[" << TimevalToStr(tvw, _is_simple_tp) << "]"
           << "[" << level_name(level) << "|" << module << "]";
        if (_is_log_out) {
            if (_ext_out_func) {
...
void Log::handleCancel() { _cond.notify_one(); }
            ...
            while (epdf) {
                if (DT_REG == epdf->d_type) {
                    ...
                    if ((pos not_eq std::string::npos) and (".log" == ext)) {
                        ...
                        auto mod = chr::system_clock::from_time_t(mktime(file_time));

                        if (mod + chr::hours(_log_file_depth) < now) {
...
void Log::print(const Log::Level& level_, const std::string& module_, const std::string& message_) {
    ...
    if (_is_run and _toggle_levels[static_cast<size_t>(level_)]) {
        ...
        if (_queue.size() < LOG_MAX_QUEUE_SIZE) {
            _queue.push(std::make_tuple(level_, module_, message_, tvw));
        } else {
            std::string err_msg = "Log max queue size exceeded! " + std::to_string(_queue.size()) +
                                  " messages were dropped.";
            Queue q;
            _queue.swap(q);
            _queue.push(std::make_tuple(ERROR, "LOG::print", err_msg, tvw));
        }
        _cond.notify_one();
    }
}
...
void Log::start() {
    ...
    _is_run = true;
    _thread = std::make_unique<std::thread>(std::bind(&Log::execute, this));
}
...
void Log::toggleTimePoint(bool is_on_) { _is_simple_tp = is_on_; }


bool Log::isLogOutFile() const { return _is_log_out_file; }
////////////////////////////////////////////////////////////////////////////////////////////////////


LogSequence::Head::Next::Next(const Next& next_) : _stream(next_._stream) {}


LogSequence::Head::Head(const Head& head_)
    : _stream(head_._stream), _level(head_._level), _module(head_._module) {}


LogSequence::LogSequence(const Log::Level& level_, const std::string& module_)
    : _level(level_), _module(module_) {}
