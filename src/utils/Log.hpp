...
public:
    ...
    void toggleTimePoint(bool is_on_);
...
struct LogSequence {
    struct Head {
        struct Next {
            ...
            template <class Type>
            explicit Next(std::stringstream* stream_, const Type& value_) : _stream(stream_) {
                (*_stream) << value_;
            }
        ...
        explicit Head(const Log::Level& level_, const std::string& module_, const Type& value_)
            : _stream(new std::stringstream), _level(level_), _module(module_) {
            (*_stream) << value_;
        }

        Head(const Head& head_);

        ~Head() { Singleton<Log>::get()->print(_level, _module, _stream->str().c_str()); }
...
#define LOG_TO_FUNC(func, full_out) \
    utils::Singleton<utils::Log>::get()->initExtFunc((func), (full_out));
...
#define LOG_SIMPLE_TIME_POINT(is_on) utils::Singleton<utils::Log>::get()->toggleTimePoint((is_on));

#define LOG_FLUSH utils::Singleton<utils::Log>::get()->flush();
